...
1. Add setup metadata

   Add `execution-mode`, `target-os`, and `script-directory` inputs to
   `action.yml` and `setup/action.yml`.  Add `setup-script` and `setup-env`
   outputs.  Default run mode must preserve current behavior.

2. Parse setup execution inputs
...
4. Add POSIX script rendering helpers
...
6. Emit binary source environment
...
7. Emit vcpkg bootstrap and NuGet fetch
...
9. Emit NuGet source configuration
...
10. Add setup summaries and diagnostics
...
12. Verify with a FreeBSD workflow
