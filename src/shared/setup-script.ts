...
export function renderSetupScript(
  ...
  script.line("}");
  script.blank();
  script.line("find_bsd_libcurl() {");
  if (bsdLibcurlCompatTarget) {
    if (bsdTarget.targetOs === "netbsd") {
      script.line("  for candidate in \\");
      script.line("      /usr/lib/libcurl.so.4 \\");
      script.line("      /usr/lib/libcurl.so.* \\");
      script.line("      /usr/lib/libcurl.so \\");
      script.line("      /usr/pkg/lib/libcurl.so.4 \\");
      script.line("      /usr/pkg/lib/libcurl.so.* \\");
      script.line("      /usr/pkg/lib/libcurl.so; do");
    } else {
      script.line("  for candidate in \\");
      script.line("      /usr/local/lib/libcurl.so.4 \\");
      script.line("      /usr/local/lib/libcurl.so.* \\");
      script.line("      /usr/local/lib/libcurl.so; do");
    }
    script.line('    if [ -f "${candidate}" ]; then');
  ...
  if (bsdLibcurlCompatTarget) {
    script.line(
      "  if ! command_exists curl || ! find_bsd_libcurl >/dev/null 2>&1; then",
    );
  } else {
  ...
  if (bsdTarget.targetOs === "netbsd") {
    ...
    script.line(
      "      [ ! -f /usr/pkg/share/mozilla-rootcerts/cacert.pem ]; then",
    );
    script.command("    printf", [
      posixLiteral("%s\\n"),
  ...
  if (bsdLibcurlCompatTarget) {
    script.line("  libcurl_file=$(find_bsd_libcurl || true)");
    script.line('  if [ -z "${libcurl_file}" ]; then');
    script.command("    printf", [
      posixLiteral("%s\\n"),
      posixLiteral(
        `${bsdTarget.label} libcurl library not found; install curl`,
      ),
    ]);
    script.line("    exit 1");
    script.line("  fi");
    ...
    script.line('  libcurl_dir="${libcurl_file%/*}"');
    script.line('  bsd_library_path="${compat_dir}:${libcurl_dir}"');
    script.line('  if [ -z "${LD_LIBRARY_PATH:-}" ]; then');
    script.line('    LD_LIBRARY_PATH="${bsd_library_path}"');
    script.line("  else");
    script.line('    LD_LIBRARY_PATH="${bsd_library_path}:${LD_LIBRARY_PATH}"');
    script.line("  fi");
    script.line("  export LD_LIBRARY_PATH");
    script.command("  printf", [
      posixLiteral("%s%s\\n"),
      posixLiteral(`${bsdTarget.label} libcurl compatibility path: `),
      posixRuntimeExpression('"${compat_dir}"'),
    ]);
    script.line("  unset compat_root bsd_library_path libcurl_dir");
  } else {
    script.line("  :");
  }
  script.line("}");
  ...
  if (bsdTarget.targetOs === "netbsd") {
    ...
    script.line(
      "  cat > \"${toolchain_file}\" <<'VCPKG_GITHUB_CACHE_NETBSD_TOOLCHAIN'",
    );
  ...
  if (bsdTarget.targetOs === "netbsd") {
    ...
    script.command("    printf", [
      posixLiteral("%s\\n"),
      posixLiteral(
        "NetBSD vcpkg-tool bootstrap patch skipped: bootstrap helper missing",
      ),
    ]);
    script.line("    return");
    script.line("  fi");
    script.line(
      '  if grep -q "vcpkg-github-cache NetBSD vcpkg-tool isfinite patch" "${bootstrap_file}"; then',
    );
    script.command("    printf", [
      posixLiteral("%s\\n"),
    ...
    script.line(
      '        print "    if [ \\"$(uname -s)\\" = \\"NetBSD\\" ]; then"',
    );
    script.line(
      '        print "        json_cpp=\\"$srcDir/src/vcpkg/base/json.cpp\\""',
    );
    script.line('        print "        if [ -f \\"$json_cpp\\" ]; then"');
    script.line(
      '        print "            tmp_json_cpp=\\"${json_cpp}.vcpkg-github-cache.tmp\\""',
    );
    script.line(
      '        print "            if sed \\"s/check_exit(VCPKG_LINE_INFO, isfinite(d))/check_exit(VCPKG_LINE_INFO, std::isfinite(d))/\\" \\"$json_cpp\\" > \\"$tmp_json_cpp\\" && ! cmp -s \\"$json_cpp\\" \\"$tmp_json_cpp\\"; then"',
    );
    script.line(
      '        print "                mv \\"$tmp_json_cpp\\" \\"$json_cpp\\""',
    );
    script.line(
    ...
    script.line('        print "        fi"');
    script.line(
      '        print "        test_json_cpp=\\"$srcDir/src/vcpkg-test/json.cpp\\""',
    );
    ...
    script.line('        print "            else"');
    script.line(
      '        print "                rm -f \\"$tmp_test_json_cpp\\""',
    );
    script.line(
      '        print "                printf \\"%s\\\\n\\" \\"NetBSD vcpkg-tool test signbit patch skipped\\""',
    );
    script.line('        print "            fi"');
    script.line('        print "        else"');
    script.line(
      '        print "            printf \\"%s\\\\n\\" \\"NetBSD vcpkg-tool test signbit patch skipped: json.cpp missing\\""',
    );
    script.line('        print "        fi"');
    script.line('        print "    fi"');
    script.line("        patched = 1");
  ...
  script.line('  if [ -n "${bsd_nuget_version:-}" ] &&');
  script.line('      [ -n "${bsd_nuget_url:-}" ] &&');
  ...
  script.command("  printf", [
    posixLiteral("%s%s\\n"),
    posixLiteral(`${bsdTarget.label} NuGet version: `),
    posixRuntimeExpression('"${bsd_nuget_version}"'),
  ]);
  script.line("}");
  script.blank();
  ...
  script.line("  set_bsd_nuget_metadata");
  script.line(
    `  nuget_dir="\${VCPKG_ROOT}/downloads/tools/nuget-\${bsd_nuget_version}-${bsdTarget.nugetDirectorySuffix}"`,
  );
  ...
  script.line('    curl -L -o "${nuget_exe}.tmp" "${bsd_nuget_url}"');
  script.line('    actual_hash=$(sha512_file "${nuget_exe}.tmp")');
  script.line('    expected_hash="${bsd_nuget_sha512}"');
...
export function renderSetupEnvironment(
  ...
  const targetSettings = bsdTargetSettings(plan.targetOs);
  const bsdLibcurlCompatTarget =
    targetSettings?.targetOs === "netbsd" ||
    targetSettings?.targetOs === "openbsd";
  const bsdNativeLibcurlDirectory =
    targetSettings?.targetOs === "netbsd" ? "/usr/pkg/lib" : "/usr/local/lib";
  ...
  if (bsdLibcurlCompatTarget) {
    ...
    script.line("fi");
    script.line('case "${VCPKG_ROOT}" in');
    script.line('  /*) bsd_vcpkg_root="${VCPKG_ROOT}" ;;');
    script.line('  *) bsd_vcpkg_root="$(pwd -P)/${VCPKG_ROOT}" ;;');
    script.line("esac");
    script.line(
      'bsd_libcurl_dir="${bsd_vcpkg_root}/buildtrees/vcpkg-github-cache/lib"',
    );
    script.line(
      `bsd_library_path=${quotePosixShellLiteral(bsdNativeLibcurlDirectory)}`,
    );
    script.line('if [ -d "${bsd_libcurl_dir}" ]; then');
    script.line('  bsd_library_path="${bsd_libcurl_dir}:${bsd_library_path}"');
    script.line("fi");
    script.line('if [ -z "${LD_LIBRARY_PATH:-}" ]; then');
    script.line('  export LD_LIBRARY_PATH="${bsd_library_path}"');
    script.line("else");
    script.line(
      '  export LD_LIBRARY_PATH="${bsd_library_path}:${LD_LIBRARY_PATH}"',
    );
    script.line("fi");
    script.line("unset bsd_libcurl_dir bsd_library_path bsd_vcpkg_root");
  }
